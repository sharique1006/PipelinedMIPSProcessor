...
//initailzed an array to store if a register is to be written in so that Hazard can be detected and Data can be forwarded
//Store the DataForwarding for that register in an array
int inWriteRegArr[32];
float DataForwardingReg[32];

//Function to check if it was a hit or a miss
int hit_miss (float x) {
	//int n = rand() % 101;
...
INSTRUCTION ID(struct INSTRUCTION instruct) {
	...
	if(strcmp(tokenized_inst[0], "lw") == 0 || strcmp(tokenized_inst[0], "sw") == 0) {
		...
		//We have to load the value in register rt so hazard can be found in the next instruction of a value is a read from it hence store this
	...
	//when add, sub - assigned values to rd, rs, rt and rd
		...
		instruct.shamt = 0;
		instruct.offset = 0;
		//We have to load the value in register rd so hazard can be found in the next instruction of a value is a read from it hence store this
...
tuple<string, int, int, float> EX(struct INSTRUCTION instruct) {
	...
	if(instruct.function == "lw") {
		//Store the value from memory since a hazard can be found in the next step so we may have to forward the data
	...
	//return the result after adding the values
	else if(instruct.function == "add") {
		float result;
		//if hazard is found use the Data Forward to evaluate the result
		...
		//In case of no hazard use the register values
		else {
	...
	//return th result after shifting right
		...
		//If hazard found use the value from DataForwarding
		if(inWriteRegArr[instruct.rt] == 1) {
			if(register_file[instruct.rs] == DataForwardingReg[instruct.rt]) {
				PC = instruct.offset;
				return make_tuple("beq", 0, instruct.offset, 0.0);
			}
		}
		else if(inWriteRegArr[instruct.rs] == 1) {
			if(DataForwardingReg[instruct.rs] == DataForwardingReg[instruct.rt]) {
				PC = instruct.offset;
				return make_tuple("beq", 0, instruct.offset, 0.0);
			}
		}
		//If no hazard use the value from register to comapre
		else {
			if(register_file[instruct.rs] == DataForwardingReg[instruct.rt]) {
				PC = instruct.offset;
				return make_tuple("beq", 0, instruct.offset, 0.0);
		...
		//If hazard found use the value from DataForwarding
		if(inWriteRegArr[instruct.rs] == 1) {
			if(DataForwardingReg[instruct.rs] <= 0) {
				PC = instruct.offset;
		...
		//If no hazard use the values from the register
		else {
			if(register_file[instruct.rs] > 0) {
				PC = instruct.offset;
				return make_tuple("bgtz", 0, instruct.offset, 0.0);
...
//If it was a miss stall the pipeline for N-1 clock cycles
void STALL(int N, struct INSTRUCTION initialInstr) {
	pipeline_array[0] = initialInstr;
	int k = 0;
	int sPC = PC;
	//Stall the pipeline for N-1 cycles
	while(k != N-1) {
		clock_cycles++;
		cout <<"CLOCK CYCLE : "<<clock_cycles<<endl;
		pipeline_array[1] = ID(pipeline_array[1]);
...
int main(int argc, char* argv[]) {
	...
	infile.open(argv[1]);
	//Read the probability value and number of clock cycles it would take in case of a miss
	float x = atof(argv[2]);
	float N = atof(argv[3]);
	string str;
	int index = 0;
	...
	while(pipeline_array[0].instruction != "exit") {
		...
		//Check for a miss or a hit in case of lw
		else if (idex.function == "lw") {
			int hit = hit_miss(x);
			//If hit move to next PC value
			if(hit == 1) {
				cout <<"HIT"<<endl;
				PC++;
			}
			//If miss stall the pipeline for N-1 clock cycles
			else {
				cout <<"MISS"<<endl;
				STALL(N, initialInstr);
			}
		}
		//Else increase the Program Counter
		else {
			PC++;
		}
